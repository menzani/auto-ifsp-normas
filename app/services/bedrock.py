...
def _structure_chunk(chunk: str, is_continuation: bool) -> str:
    ...
    prompt = (
        ...
        "2. ESTRUTURE em Markdown seguindo rigorosamente a hierarquia abaixo:\n\n"
        "   # (H1) — Título do normativo (ex: 'TÍTULO I — DAS DISPOSIÇÕES GERAIS').\n"
        "      Use apenas se o documento tiver divisão em Títulos numerados.\n\n"
        "   ## (H2) — Capítulos (ex: 'CAPÍTULO I — DO OBJETIVO',\n"
        "      'CAPÍTULO II — DO ÂMBITO DE APLICAÇÃO').\n"
        "      IMPORTANTE: inclua o nome do capítulo no mesmo heading quando presente\n"
        "      (ex: '## CAPÍTULO I — DO OBJETIVO', não separe em duas linhas).\n\n"
        "   ### (H3) — Seções dentro de capítulos (ex: 'Seção I — Da Aplicação').\n\n"
        "   Artigos (Art. 1º, Art. 2º…), parágrafos (§ 1º, Parágrafo único) e\n"
        "   incisos (I, II, III…) NÃO recebem heading — são parágrafo de texto normal.\n\n"
        "   NÃO adicione heading em:\n"
        "   - Timbres institucionais ('MINISTÉRIO DA EDUCAÇÃO', 'INSTITUTO FEDERAL…')\n"
        "   - Linhas de atribuição ('O REITOR', 'O DIRETOR', 'A REITORA')\n"
